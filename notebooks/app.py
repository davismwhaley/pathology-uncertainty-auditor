...
# --- PAGE 2: CLUSTER INSPECTOR ---
    ...
    selected_cluster = st.selectbox("Select a Cluster ID:", cluster_ids, index=cluster_ids.index(53) if 53 in cluster_ids else 0)
    ...
    if 'query_idx' in st.session_state:
        ...
        with c2:
            ...
            for i, n_idx in enumerate(neighbor_indices):
                ...
                # Note: 'indices' from sklearn returns position in array, which matches df if not shuffled
                n_row = df.iloc[n_idx]
