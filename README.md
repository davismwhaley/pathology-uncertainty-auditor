...
**This project is not about building a classifier to replace doctors. It is about building an Auditor to protect them.** By mapping the "Ambiguous Manifold" of the BreaKHis dataset (7,909 images), I identify exactly where the boundary blurs, creating a safety net for diagnostic uncertainty.

---

## 📊 Key Findings: The 47% Ambiguity
I audited the BreaKHis dataset using unsupervised manifold learning to see if the data naturally separated into "Benign" and "Malignant."
...
*   **The "Trap" Validated:** I trained a supervised Logistic Regression classifier (94.4% Accuracy) to test this finding. **56.2% of all classification errors** occurred strictly within the "Noise" region I identified. This proves the ambiguity is objective, not random.
...
I treated the images as data points on a high-dimensional manifold rather than simple pixels.

### Phase 1: Feature Extraction (ResNet50)
I utilized a **ResNet50** architecture pre-trained on ImageNet. I removed the final classification head to use the network purely as a feature extractor.
...
High-dimensional data is impossible to visualize. I used **UMAP (Uniform Manifold Approximation and Projection)** to project the 2048D vectors into 2D space.
...
I applied **HDBSCAN**, a density-based clustering algorithm, to identify natural groupings.
*   Unlike K-Means (which forces every point into a group), HDBSCAN allows for **"Noise"** (Cluster -1).
*   I hypothesized that "Noise" points represented the biological transition zones.
*   I calculated **Shannon Entropy** for each cluster to find "Impure" regions where Benign and Malignant samples mixed.

### Phase 4: Validation & Tooling
*   **Triangulation:** I validated the findings by running a supervised stress test. The errors aligned perfectly with our unsupervised "Noise" clusters.
*   **Reverse Image Search:** I built a Content-Based Image Retrieval (CBIR) tool using `NearestNeighbors` to allow pathologists to query the manifold directly.
